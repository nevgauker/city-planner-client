        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {trips.map((trip) => {
              ...
              return (
                <motion.div key={trip.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="bg-white/10 border border-white/20 rounded-lg p-4 flex items-center justify-between hover:bg-white/15 transition-colors">
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-white truncate">{trip.title || trip.city}</h3>
                    <p className="text-sm text-white/60">{trip.city} • {formattedDate}</p>
                    {trip.startDate && trip.endDate && (
                      <p className="text-xs text-white/40 mt-1">{new Date(trip.startDate).toLocaleDateString()} – {new Date(trip.endDate).toLocaleDateString()}</p>
                    )}
                  </div>
