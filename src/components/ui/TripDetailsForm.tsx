...
export default function TripDetailsForm({
  ...
  return (
    <div className="glass-effect card-elevation rounded-lg p-4 sm:p-8 space-y-6 sm:space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
              {Object.keys(stylePresets).map((preset) => (
                <motion.button key={preset} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => onStylePreset(preset)}
                  className={`px-3 py-2 text-xs rounded font-medium transition-all ${JSON.stringify(formData.travelStyles) === JSON.stringify(stylePresets![preset]) ? 'bg-blue-600 text-white' : 'bg-white/10 text-white hover:bg-white/20 border border-white/20'}`}>
                  {preset}
                </motion.button>
              ))}
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {travelStyles.map((style) => (
            <motion.button key={style} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => onTravelStyleToggle(style)}
              className={`px-4 py-2 rounded-lg font-medium transition-all ${formData.travelStyles.includes(style) ? 'bg-warm-accent text-navy-900' : 'bg-white/10 text-white hover:bg-white/20 border border-white/20'}`}>
              {style}
            </motion.button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {paceOptions.map((pace) => (
            <motion.button key={pace} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => onPaceChange(pace)}
              className={`px-4 py-2 rounded-lg font-medium transition-all ${formData.pace === pace ? 'bg-warm-accent text-navy-900' : 'bg-white/10 text-white hover:bg-white/20 border border-white/20'}`}>
              {pace}
            </motion.button>
          ))}
        </div>
      ...
      <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={onSubmit} className="w-full px-4 sm:px-6 py-3 bg-warm-accent hover:bg-warm-light text-navy-900 font-bold rounded-lg transition-colors text-base sm:text-lg">
        Plan My Trip ✈️
      </motion.button>
    </div>
  )
